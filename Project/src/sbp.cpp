 ...
 */
 ...
 void create_1D_D1_operator_sparse(
    SparseMatrixCOO **D1, SparseMatrixCOO **H, SparseMatrixCOO **HI,
    ...
    *D1 = create_coo_matrix(Nxp, Nxp, nnz);
    *H = create_coo_matrix(Nxp, Nxp, Nxp);
    *HI = create_coo_matrix(Nxp, Nxp, Nxp);
    ...
    for (unsigned int i = 0; i < Nxp; i++){
        ...
        if (i < bm){
            (*H)->values[i] = (*H)->values[i]  / bhinv[i];
        }

        if (i > Nxp - bm - 1){
            (*H)->values[i] = (*H)->values[i]  / bhinv[Nxp - i - 1];
        }
        (*HI)->values[i] = 1.0f / (*H)->values[i];
        (*HI)->row_indices[i] = i;
        (*HI)->col_indices[i] = i;
    }
    ...
    for (unsigned int i = 0; i < Nxp; i++){
        ...
        if (i < bm){
            for (unsigned int j = 0; j < bn; j++){
                (*D1)->values[idx] = bd[i*bn + j] / h;
                (*D1)->row_indices[idx] = i;
                (*D1)->col_indices[idx] = j;
                idx++;
            }
        }
            ...
            for (unsigned int j = 0; j < bn; j++){
                (*D1)->values[idx] = -1.0 * bd[(Nxp - i - 1)*bn + (bn - j - 1)] / h;
                (*D1)->row_indices[idx] = i;
                (*D1)->col_indices[idx] = (Nxp + j - bn);
                idx++;
            }
        }
        else {
            for (unsigned int j=0; j<interior; j++){
                (*D1)->values[idx] =  d[j] / h;
                (*D1)->row_indices[idx] = i;
                (*D1)->col_indices[idx] = i +  j - (interior / 2);
                idx++;
            }
        }
...
void create_1D_D2_operator_sparse(SparseMatrixCOO **D2, SparseMatrixCOO **BS, 
    ...
    *D2 = create_coo_matrix(Nxp, Nxp, nnz);
    *BS = create_coo_matrix(Nxp, Nxp, 2*bs);
    ...
    for (unsigned int i = 0; i < Nxp; i++){
        ...
        if (i < bm){
            for (unsigned int j = 0; j < bn; j++){
                //printf("at index %u %u d2 = %.2f\n", i, j,  bd[i*bn + j] / h2);
                (*D2)->values[idx] = bd[i*bn + j] / h2;
                (*D2)->row_indices[idx] = i;
                (*D2)->col_indices[idx] = j;
                idx++;
            }
        }
            ...
            for (unsigned int j = 0; j < bn; j++){
                printf("IDX: %u\n", idx);
                (*D2)->values[idx] = 1.0 * bd[(Nxp - i - 1)*bn + (bn - j - 1)] / h2;
                (*D2)->row_indices[idx] = i;
                (*D2)->col_indices[idx] = (Nxp - bn + j);
                idx++;
            }
        }
        else {
            for (unsigned int j=0; j<interior; j++){
                (*D2)->values[idx] =  d[j] / h2;
                (*D2)->row_indices[idx] = i;
                (*D2)->col_indices[idx]= i + j - (interior / 2);
                idx++;
            }
        }
    ...
    for (unsigned int i = 0; i < bs; i++){
        ...
        (*BS)->col_indices[i] = i;
        // negative side
        (*BS)->values[end_idx - i - 1] = bsd[i]/h;
        (*BS)->row_indices[end_idx - i - 1] = Nxp - 1;
        (*BS)->col_indices[end_idx - i - 1] = Nxp - i - 1;
    }
}


void addSAT_dir_sparse(SparseMatrixCOO ** SAT, SparseMatrixCOO *BS, unsigned int Nx, SparseMatrixCOO *HI, double mu){

    unsigned int Nxp = Nx + 1;
    // Allocate Memory for Operators
    *SAT = create_coo_matrix(Nxp, Nxp, BS->nnz); // SAT Term will only have as many elements as BS
    SparseMatrixCOO *E0 = create_coo_matrix(Nxp, Nxp, 1);
    SparseMatrixCOO *EN = create_coo_matrix(Nxp, Nxp, 1);

    E0->values[0] = 1; E0->row_indices[0] = 0; E0->col_indices[0] = 0;
    EN->values[0] = 1; EN->row_indices[0] = Nx; EN->col_indices[0] = Nx;
    ...
    double alpha1 = -13.0 * HI->values[0];
    double beta = 1.0;

    // Goal:
    // alpha1 mu HI en + beta HI (muBS)^T en
    // mu HI(alpha1 I + betaBS^T)EN

    // Term 1
    SparseMatrixCOO *SAT1 = liftOutSparse(HI, EN);
    matrixMulScalarIPSparse(SAT1, alpha1*mu);

    //term 2
    SparseMatrixCOO *BST = matrixTransposeSparse(BS);
